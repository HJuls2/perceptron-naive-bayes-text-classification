...
def calc_metrics(categories,y_true,y_true_by_class,y_pred,scores):
    precision=recall=dict()
    score=dict()
    for cat in categories:
        score[cat]=np.array([np.power(s[1][categories.index(cat)],10) for s in scores.items()])
        print(score[cat])
        precision[cat],recall[cat],_=precision_recall_curve(y_true_by_class[cat],score[cat])
    return precision,recall,f1_score(y_true,y_pred,average=None)
